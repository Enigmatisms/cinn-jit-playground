...
{
  __builtin_assume(((int)blockIdx.x < 7));
  ...
  for (int32_t reduce_k_0_0_reduce_k_1_0_reduce_k_2_0_fused = 0; reduce_k_0_0_reduce_k_1_0_reduce_k_2_0_fused < 1152; reduce_k_0_0_reduce_k_1_0_reduce_k_2_0_fused += 1) {
    float var_local = var[((((((reduce_k_0_0_reduce_k_1_0_reduce_k_2_0_fused * 128) + (int)threadIdx.y) + ((int)blockIdx.y * 16)) * 224) + ((int)blockIdx.x * 32)) + (int)threadIdx.x)];
    var_0_rf[0] = (var_0_rf[0] + var_local);
    var_3_rf[0] = (var_3_rf[0] + (var_local * var_local));
  };
  var_3_rf_0__reduce_init[((((int)blockIdx.x * 32) + (int)threadIdx.x) + ((int)blockIdx.y * 224))] = 0.00000000f;
  var_3_rf_0[((((int)blockIdx.x * 32) + (int)threadIdx.x) + ((int)blockIdx.y * 224))] = cinn_discrete_reduce_sum_fp32(var_3_rf[0], shm32__fp32_reduce);
  var_0_rf_0__reduce_init[((((int)blockIdx.x * 32) + (int)threadIdx.x) + ((int)blockIdx.y * 224))] = 0.00000000f;
  var_0_rf_0[((((int)blockIdx.x * 32) + (int)threadIdx.x) + ((int)blockIdx.y * 224))] = cinn_discrete_reduce_sum_fp32(var_0_rf[0], shm32__fp32_reduce);
  is_last_block_done[0] = cinn_grid_reduce_update_semaphore(semaphore);
  if (is_last_block_done[0]) {
    var_3[(((int)blockIdx.x * 32) + (int)threadIdx.x)] = cinn_grid_reduce_sum_fp32(var_3_rf_0, 224, (((int)blockIdx.x * 32) + (int)threadIdx.x));
    var_0[(((int)blockIdx.x * 32) + (int)threadIdx.x)] = cinn_grid_reduce_sum_fp32(var_0_rf_0, 224, (((int)blockIdx.x * 32) + (int)threadIdx.x));
  };
...
template <int C>
__global__
void __launch_bounds__(256) kernel_64_48_224_shared_full(
...
int main()
{
    ...
    constexpr size_t N = 64, H = 48, W = 48, C = 224;
    ...
    Tensor<float> var_0_rf(1792, Zero<float>{});
    Tensor<float> var_3_rf(1792, Zero<float>{});
    Tensor<int> semaphores(32, Zero<int>{});

    kernel_reduce<<<dim3(7, 8), dim3(32, 16), 2048>>>(X, var1, var0, var3, var_0_rf, var_3_rf, semaphores);
    ...
    kernel_64_48_224_shared_full<C><<<32256, 256, dynamic_shared>>>(var0, var3, X, var1, out2, w1, w2, out3, out4);
    kernel_64_48_224<<<32256, 256>>>(var0, var3, X, var1_cp, out2_cp, w1, w2, out3_cp, out4_cp);
    CUDA_CHECK_RETURN(cudaDeviceSynchronize());
